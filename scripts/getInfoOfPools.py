...
def main():
    ...
    for i in range(len(newalloc)):
        ...
        pair = interface.IUniswapV2Pair(
            interface.IVaultToken(collateral.underlying()).underlying()
        )
        ...
        print(
            f"LPFunds : {reserves[0] / 10**token0.decimals()} {token0.symbol()} - {reserves[1] / 10**token1.decimals()} {token1.symbol()}"
        )
